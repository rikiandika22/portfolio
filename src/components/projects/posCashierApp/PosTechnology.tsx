...
import type { PosTechnologySlideData } from "@/data/posCashierAppSlides";
import { POS_ALL_TECH_ICONS } from "@/data/posCashierAppSlides";
...
interface PosTechnologyProps {
  data: PosTechnologySlideData;
}

/**
 * Slide 02 — 32 BLOC The Technology Behind the System.
 * Uses identical typography system, headline scale, and interactive accordion
 * components as Sumber Agung Trans.
 */
export default function PosTechnology({ data }: PosTechnologyProps) {
  // Track which Key Features are open (independent toggle, initial compact state)
  ...
  return (
    <div className="sat-technology flex flex-col h-full min-h-0 overflow-y-auto overflow-x-hidden gap-y-4 px-4 pt-4 pb-14 sm:px-8 sm:pt-6 sm:pb-14 lg:grid lg:grid-rows-[auto_auto_minmax(0,1fr)] lg:gap-y-3 lg:px-11 lg:pt-8 lg:pb-14 box-border">
      ...
      <div className="grid grid-cols-1 content-start gap-x-12 gap-y-4 lg:gap-y-0 lg:grid-cols-12 xl:gap-x-16 w-full">
        ...
        {/* Key Features Column (Interactive Accordion) */}
